...
public class MergeImageProcessor {
    ...
    public MergeImageProcessor(ChannelsDiffProcessor chDiffProc) {
        if (!chDiffProc.haveBeenProcessed()) {
            IJ.error("Image not processed");
        }
    ...
    private ImagePlus channelsMergeAlloca() {
        ImagePlus im = NewImage.createByteImage("MergeChannel", this.chDiffProc.getDiffRed().getWidth(), this.chDiffProc.getDiffRed().getHeight(), 1, NewImage.FILL_BLACK);
        return im;
    }
}
